...
class EntitySearcher {
    constructor(options) {
        ...
        this.displayExtra = options.displayExtra || null;
        this.notFoundText = options.notFoundText || '未找到匹配的结果';
    ...
    matchesKeyword(item, keyword) {
        if (this.fuzzyMatch(item[this.fieldCode], keyword) || this.fuzzyMatch(item[this.fieldName], keyword)) {
            return true;
        }
        for (const field of this.extraSearchFields) {
            if (this.fuzzyMatch(item[field] || '', keyword)) {
                return true;
            }
        }
        return false;
    ...
    renderDropdown() {
        ...
        if (this.currentResults.length === 0) {
            this.dropdown.html(`<div class="p-2 text-muted">${this.notFoundText}</div>`);
        } else {
            const items = this.currentResults.map((item, index) => {
                const extraHtml = self.displayExtra ? self.displayExtra(item) : '';
